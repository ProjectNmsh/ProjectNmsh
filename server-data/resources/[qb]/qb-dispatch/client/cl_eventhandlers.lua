...
AddEventHandler('CEventShockingGunshotFired', function(witnesses, ped, coords)
    ...
    if inHuntingZone then exports['qb-dispatch']:Hunting(); Config.Timer['Shooting'] = Config.Shooting.Success return end
    ...
    if vehicle ~= 0 then
        ...
        exports['qb-dispatch']:VehicleShooting(vehicle, ped, coords)
        Config.Timer['Shooting'] = Config.Shooting.Success
    else
        exports['qb-dispatch']:Shooting(ped, coords)
        Config.Timer['Shooting'] = Config.Shooting.Success
    end
end)
    ...
    AddEventHandler(event, function(witnesses, ped, coords)
        ...
        if (GetEntitySpeed(vehicle) * 3.6) >= (120 + RandomNum(0, 20)) then
            ...
            if ((GetEntitySpeed(vehicle) * 3.6) >= 90) then
                vehicle = vehicleData(vehicle)
                exports['qb-dispatch']:SpeedingVehicle(vehicle, ped, coords)
...
AddEventHandler('CEventShockingSeenMeleeAction', function(witnesses, attacker, coords)
    ...
    exports['qb-dispatch']:Fight(attacker, coords)
...
AddEventHandler('CEventPedJackingMyVehicle', function(witnesses, jacker)
    ...
    -- If the vehicle isn't whitelisted, don't trigger the event
    if not vehicleWhitelist[GetVehicleClass(vehicle)] then return end 
    exports['qb-dispatch']:CarJacking(vehicle, jacker)
...
AddEventHandler('CEventShockingCarAlarm', function(witnesses, thief, coords)
    ...
    -- If the vehicle isn't whitelisted, don't trigger the event
    if not vehicleWhitelist[GetVehicleClass(vehicle)] then return end 
    exports['qb-dispatch']:VehicleTheft(vehicle, thief, coords)
    Config.Timer['Autotheft'] = Config.Autotheft.Success
end)
...
AddEventHandler('gameEventTriggered', function(name, args)
    ...
    if not Config.AuthorizedJobs.FirstResponder.Check() then exports['qb-dispatch']:InjuriedPerson() end
    if Config.AuthorizedJobs.LEO.Check() then exports['qb-dispatch']:OfficerDown() end
    if Config.AuthorizedJobs.EMS.Check() then exports['qb-dispatch']:EmsDown() end
