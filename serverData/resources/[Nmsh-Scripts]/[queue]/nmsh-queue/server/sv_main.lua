    ...
    for license, data in pairs(playerConnecting) do 
        ...
        if GetPlayerName(user) == nil or GetPlayerName(user) ~= name then 
            ...
            if Config.Debug then 
                print("[nmsh-queue] (Thread : NO LONGER EXISTS) Popped player " .. name .. " from queue...");
            end
            ...
            if Config.Debug then 
                print("[nmsh-queue] (Thread : NO LONGER EXISTS) currentConnectors is == " .. tostring(currentConnectors) )
            end
        ...
        if Config.onlyActiveWhenFull == true then 
            ...
            if (GetPlayerCount() + 1) > slots or (GetPlayerCount() + Queue:GetMax()) > slots then  
                ...
                if connecting[license] == nil or connecting[license] ~= true then 
                    ...
                    if Config.Debug then 
                        print("[nmsh-queue] currentConnectors is == " .. tostring(currentConnectors) )
                    end
            ...
            if connecting[license] == nil or connecting[license] ~= true then 
                ...
                if Config.Debug then 
                    print("[nmsh-queue] Total Connected Players: " .. tostring(currentConnectors) )
                end
            end -- connecting[license] == nil
            deferrals.done();
        end
...
RegisterNetEvent('nmsh-queue/server/activated', function()
