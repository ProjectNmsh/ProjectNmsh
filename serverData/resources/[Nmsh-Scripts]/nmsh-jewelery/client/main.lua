...
local function smashVitrine(k)
    ...
    QBCore.Functions.TriggerCallback('nmsh-jewellery:server:getCops', function(cops)
        if cops >= Config.RequiredCops then
            ...
            QBCore.Functions.Progressbar("smash_vitrine", Lang:t('info.progressbar'), Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
                ...
                TriggerServerEvent('nmsh-jewellery:server:vitrineReward', k)
                TriggerServerEvent('nmsh-jewellery:server:setTimeout')
                TriggerServerEvent('police:server:policeAlert', 'Robbery in progress')
                smashing = false
                TaskPlayAnim(ped, animDict, "exit", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end, function() -- Cancel
                TriggerServerEvent('nmsh-jewellery:server:setVitrineState', "isBusy", false, k)
                smashing = false
                TaskPlayAnim(ped, animDict, "exit", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end)
            TriggerServerEvent('nmsh-jewellery:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	QBCore.Functions.TriggerCallback('nmsh-jewellery:server:getVitrineState', function(result)
		Config.Locations = result
	end)
end)

RegisterNetEvent('nmsh-jewellery:client:setVitrineState', function(stateType, state, k)
...
CreateThread(function()
    if Config.UseTarget then
        for k, v in pairs(Config.Locations) do
            exports["nmsh-target"]:AddBoxZone("jewelstore" .. k, v.coords, 1, 1, {
                name = "jewelstore" .. k,
                heading = 40,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
