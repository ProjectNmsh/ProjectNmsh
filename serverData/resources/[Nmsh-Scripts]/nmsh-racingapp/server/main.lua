...
local function giveSplit(src, racers, position, pot)
    ...
    if total > 0 then
        if Config.Options.MoneyType == 'crypto' and Config.UseRenewedCrypto then
            exports['nmsh-phone']:AddCrypto(src, Config.Options.CryptoType, math.floor(total))
...
local function handOutParticipationTrophy(src, position)
    ...
    if Config.ParticpationTrophies.amount[position] then
        if Config.ParticpationTrophies.type == 'crypto' and Config.UseRenewedCrypto then
            exports['nmsh-phone']:AddCrypto(src, Config.ParticpationTrophies.cryptoType, Config.ParticpationTrophies.amount[position])
...
---- Server Events ----
-----------------------
RegisterNetEvent('nmsh-racingapp:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap, CarClass, VehicleModel)
    local src = source
    ...
    TriggerClientEvent('nmsh-racingapp:client:PlayerFinish', -1, RaceData.RaceId, PlayersFinished, RacerName)
...
RegisterNetEvent('nmsh-racingapp:server:CreateLapRace', function(RaceName, RacerName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if IsPermissioned(Player.PlayerData.citizenid, 'create') then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('nmsh-racingapp:client:StartRaceEditor', source, RaceName, RacerName)
        else
...
local function hasEnoughMoney(source, cost)
    if Config.Options.MoneyType == 'crypto' and Config.UseRenewedCrypto then
        ...
        if exports['nmsh-phone']:hasEnough(source, Config.Options.CryptoType, math.floor(cost)) then
            return true
        else
...
RegisterNetEvent('nmsh-racingapp:server:JoinRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if isToFarAway(src, RaceId) then
        TriggerClientEvent('nmsh-racingapp:client:NotCloseEnough', src, Races[RaceId].Checkpoints[1].coords.x, Races[RaceId].Checkpoints[1].coords.y)
        return
    end
        ...
        if RaceData.BuyIn > 0 and not hasEnoughMoney(src, RaceData.BuyIn) then
            ...
            if CurrentRace ~= nil then
                ...
                if (AmountOfRacers - 1) == 0 then
                    ...
                    TriggerClientEvent('nmsh-racingapp:client:LeaveRace', src, Races[CurrentRace])
                else
            ...
            if RaceData.BuyIn > 0 then
                if Config.Options.MoneyType == 'crypto' and Config.UseRenewedCrypto then
                    exports['nmsh-phone']:RemoveCrypto(src, Config.Options.CryptoType, math.floor(RaceData.BuyIn))
            ...
            TriggerClientEvent('nmsh-racingapp:client:JoinRace', src, Races[RaceId], RaceData.Laps, RacerName)
            TriggerClientEvent('nmsh-racingapp:client:UpdateRaceRacers', -1, RaceId, Races[RaceId].Racers)
...
local function assignNewOrganizer(RaceId, src)
    for i,v in pairs(Races[RaceId].Racers) do
        if i ~= QBCore.Functions.GetPlayer(src).PlayerData.citizenid then
            ...
            TriggerClientEvent('nmsh-racingapp:client:UpdateOrganizer', -1, RaceId, i)
            return
        end
    end
end

RegisterNetEvent('nmsh-racingapp:server:LeaveRace', function(RaceData)
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('QBCore:Notify', src, Lang:t("primary.race_last_person"))
        TriggerClientEvent('nmsh-racingapp:client:LeaveRace', src, Races[RaceId])
...
RegisterNetEvent('nmsh-racingapp:server:SetupRace', function(RaceId, Laps, RacerName, MaxClass, GhostingEnabled, GhostingTime, BuyIn)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if isToFarAway(src, RaceId) then
        TriggerClientEvent('nmsh-racingapp:client:NotCloseEnough', src, Races[RaceId].Checkpoints[1].coords.x, Races[RaceId].Checkpoints[1].coords.y)
        ...
        if Races[RaceId] ~= nil then
            if not Races[RaceId].Waiting then
                if not Races[RaceId].Started then
                    ...
                    TriggerClientEvent('nmsh-racingapp:client:ReadyJoinRace', src, allRaceData)
                            ...
                            if count < 5 * 60 then
                                ...
                                for cid, _ in pairs(Races[RaceId].Racers) do
                                    ...
                                    if RacerData ~= nil then
                                        ...
                                        TriggerClientEvent('nmsh-racingapp:client:LeaveRace', RacerData.PlayerData.source, Races[RaceId])
                                    end
                                end
                                table.remove(AvailableRaces, AvailableKey)
...
RegisterNetEvent('nmsh-racingapp:server:UpdateRaceState', function(RaceId, Started, Waiting)
    ...
    SetTimeout(Config.RaceResetTimer, function()
        ...
        if math.abs(GetGameTimer() - Timers[raceId]) < Config.RaceResetTimer then
            ...
            for i,racer in pairs(Races[raceId].Racers) do
                TriggerClientEvent('nmsh-racingapp:client:LeaveRace', racer.RacerSource, Races[raceId] )
...
RegisterNetEvent('nmsh-racingapp:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished, RaceTime)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Races[RaceId].Racers[CitizenId] then
        ...
        TriggerClientEvent('nmsh-racingapp:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId], newPositions)
    else
        ...
        TriggerClientEvent('nmsh-racingapp:client:LeaveRace', -1, nil)
    end
    if Config.UseResetTimer then updateTimer(RaceId) end
end)

RegisterNetEvent('nmsh-racingapp:server:StartRace', function(RaceId)
    local src = source
    ...
    for CitizenId, _ in pairs(Races[RaceId].Racers) do
        ...
        if Player ~= nil then
            TriggerClientEvent('nmsh-racingapp:client:RaceCountdown', Player.PlayerData.source,TotalRacers, newPositions)
        end
    end
    if Config.UseResetTimer then startTimer(RaceId) end
end)

RegisterNetEvent('nmsh-racingapp:server:SaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local RaceId = ''
...
RegisterNetEvent('nmsh-racingapp:Server:DeleteTrack', function(RaceId)
...
RegisterNetEvent('nmsh-racingapp:Server:ClearLeaderboard', function(RaceId)
...
QBCore.Functions.CreateCallback('nmsh-racingapp:Server:getplayers', function(_, cb)
...
function UseRacingFob(source, item)
    ...
    if Config.Inventory == 'qb' then
        if item.info.owner == citizenid then
            TriggerClientEvent('nmsh-racingapp:Client:OpenMainMenu', source, { type = item.name, name = item.info.name})
        else
            TriggerClientEvent('QBCore:Notify', source, Lang:t("error.unowned_dongle"), "error")
        end
    elseif Config.Inventory == 'ox' then
        if item.metadata.owner == citizenid then
            TriggerClientEvent('nmsh-racingapp:Client:OpenMainMenu', source, { type = item.name, name = item.metadata.name})
        else
            TriggerClientEvent('QBCore:Notify', source, Lang:t("error.unowned_dongle"), "error")
        end
    end
...
QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetRacingLeaderboards', function(source, cb, class, trackName)
    local Leaderboard = {}
...
QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetRacers', function(source, cb, RaceId)
    cb(Races[RaceId].Racers)
end)

QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetTracks', function(source, cb)
    cb(Races)
end)

QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetTrackData', function(source, cb, RaceId)
...
QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetAccess', function(source, cb, raceId)
...
RegisterNetEvent('nmsh-racingapp:server:SetAccess', function(raceId, access)
    local src = source
    if useDebug then
...
QBCore.Functions.CreateCallback('nmsh-racingapp:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(QBCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

QBCore.Functions.CreateCallback('nmsh-racingapp:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
...
QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetAmountOfTracks', function(source, cb, citizenid)
...
QBCore.Functions.CreateCallback('nmsh-racingapp:server:NameIsAvailable', function(source, cb, racerName, serverId)
    if Config.UseNameValidation then
...
QBCore.Functions.CreateCallback('nmsh-racingapp:server:GetRacerNamesByPlayer', function(source, cb, serverId)
    if Config.UseNameValidation then
        local Player = QBCore.Functions.GetPlayer(serverId)
        local citizenId = Player.PlayerData.citizenid
...
local function createRacingFob(source, citizenid, racerName, type, purchaseType)
    ...
    if purchaseType.moneyType == 'crypto' and Config.UseRenewedCrypto then
        if exports['nmsh-phone']:hasEnough(source, Config.Options.CryptoType, math.floor(cost)) then
            exports['nmsh-phone']:RemoveCrypto(source, Config.Options.CryptoType, math.floor(cost))
...
RegisterNetEvent('nmsh-racingapp:server:CreateFob', function(playerId, racerName, type, purchaseType)
...
QBCore.Commands.Add('updateracetracks', 'Update the race track table to fit nmsh-racing/nmsh-racingapp', {}, true, function(source, args)
...
QBCore.Commands.Add('cwdebugracing', 'toggle debug for racing', {}, true, function(source, args)
    ...
    TriggerClientEvent('nmsh-racingapp:client:toggleDebug',source, useDebug)
end, 'admin')
