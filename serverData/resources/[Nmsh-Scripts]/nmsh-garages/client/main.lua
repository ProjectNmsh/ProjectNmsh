...
local function MenuGarage(type, garage, indexgarage)
    ...
    exports['nmsh-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "nmsh-garages:client:VehicleList",
                args = {
                    type = type,
                    garage = garage,
                    index = indexgarage,
...
RegisterNetEvent("nmsh-garages:client:VehicleList", function(data)
    ...
    QBCore.Functions.TriggerCallback("nmsh-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            ...
            for _, v in pairs(result) do
                ...
                if type == "depot" and vname ~= nil then
                    MenuGarageOptions[#MenuGarageOptions + 1] = {
                        ...
                        params = {
                            event = "nmsh-garages:client:TakeOutDepot",
                            args = {
                                vehicle = v,
                                type = type,
                                garage = garage,
                    ...
                    MenuGarageOptions[#MenuGarageOptions + 1] = {
                        ...
                        disabled = vname == nil,
                        params = {
                            event = "nmsh-garages:client:takeOutGarage",
                            args = {
                                vehicle = v,
                                type = type,
                                garage = garage,
            ...
            exports['nmsh-menu']:openMenu(MenuGarageOptions)
        end
    end, indexgarage, type, garage.vehicle)
end)

RegisterNetEvent('nmsh-garages:client:takeOutGarage', function(data)
    local type = data.type
    local vehicle = data.vehicle
    local garage = data.garage
    local index = data.index
    QBCore.Functions.TriggerCallback('nmsh-garage:server:IsSpawnOk', function(spawn)
        if spawn then
            local location
            if type == "house" then
            ...
            QBCore.Functions.TriggerCallback('nmsh-garage:server:spawnvehicle', function(netId, properties)
                local veh = NetToVeh(netId)
                QBCore.Functions.SetVehicleProperties(veh, properties)
                exports['LegacyFuel']:SetFuel(veh, vehicle.fuel)
                doCarDamage(veh, vehicle)
                TriggerServerEvent('nmsh-garage:server:updateVehicleState', 0, vehicle.plate, index)
                closeMenuFull()
...
local function enterVehicle(veh, indexgarage, type, garage)
    ...
    if GetVehicleNumberOfPassengers(veh) == 0 then
        QBCore.Functions.TriggerCallback('nmsh-garage:server:checkOwnership', function(owned)
            if owned then
                ...
                TriggerServerEvent('nmsh-garage:server:updateVehicle', 1, totalFuel, engineDamage, bodyDamage, plate, indexgarage, type, PlayerGang.name)
                ...
                if plate then
                    TriggerServerEvent('nmsh-garages:server:UpdateOutsideVehicle', plate, nil)
                end
...
RegisterNetEvent('nmsh-garages:client:setHouseGarage', function(house, hasKey)
...
RegisterNetEvent('nmsh-garages:client:houseGarageConfig', function(garageConfig)
    Config.HouseGarages = garageConfig
end)

RegisterNetEvent('nmsh-garages:client:addHouseGarage', function(house, garageInfo)
...
RegisterNetEvent('nmsh-garages:client:TakeOutDepot', function(data)
    local vehicle = data.vehicle

    if vehicle.depotprice ~= 0 then
        TriggerServerEvent("nmsh-garage:server:PayDepotPrice", data)
    else
        TriggerEvent("nmsh-garages:client:takeOutGarage", data)
    end
end)
