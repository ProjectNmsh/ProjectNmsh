...
RegisterNetEvent('nmsh-trashsearch:client:searchtrash', function()
    if canSearch then
        ...
        for i = 1, #dumpsters do
            ...
            for i = 1, #searched do
                ...
                TriggerEvent('nmsh-trashsearch:client:progressbar',itemType)
                TriggerServerEvent('nmsh-trashsearch:server:startDumpsterTimer', dumpster)
...
RegisterNetEvent('nmsh-trashsearch:server:removeDumpster')
AddEventHandler('nmsh-trashsearch:server:removeDumpster', function(object)
    for i = 1, #searched do
...
RegisterNetEvent('nmsh-trashsearch:client:progressbar', function(itemType)
	local src = source
    ...
    QBCore.Functions.Progressbar("trash_find", "Dumpster Diving", 12000, false, true, {
        ...
        if Config.RewardTypes[itemType].type == "item" then
            ...
            TriggerServerEvent('nmsh-trashsearch:server:recieveItem')
        elseif Config.RewardTypes[itemType].type == "money" then
            QBCore.Functions.Notify("You found some cash", "success")
            QBCore.Functions.Notify("You smell like shit")
            TriggerServerEvent('nmsh-trashsearch:server:givemoney', math.random (21, 39))
...
RegisterNetEvent('nmsh-dumpsters:client:open:Dumpster:storage') 
AddEventHandler('nmsh-dumpsters:client:open:Dumpster:storage', function()
...
exports['nmsh-target']:AddTargetModel(trashCans, {
    options = {
        {
            type = "client",
            event = "nmsh-trashsearch:client:searchtrash",
            icon = "fas fa-dumpster",
            label = "Dumpster Dive",
        },
        {
            type = "client",
            event = "nmsh-dumpsters:client:open:Dumpster:storage",
