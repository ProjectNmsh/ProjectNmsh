                ...
                if dist < 20 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('nmsh-taxi:server:NpcPay', meterData.currentFare)
                            meterActive = false
                            SendNUIMessage({
                                action = "resetMeter"
                            })
...
function TaxiGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = v.label,
            params = {
                event = "nmsh-taxi:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("nmsh-taxi:client:TakeVehicle", function(data)
...
-- Events
RegisterNetEvent('nmsh-taxi:client:DoTaxiNpc', function()
...
RegisterNetEvent('nmsh-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('nmsh-taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = "toggleMeter"
        })
...
RegisterNetEvent('nmsh-taxi:client:toggleMuis', function()
    Wait(400)
    if meterIsOpen then
...
RegisterNetEvent('nmsh-taxijob:client:requestcab', function()
    TaxiGarage()
end)
        ...
        exports['nmsh-target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = "client",
                        event = "nmsh-taxijob:client:requestcab",
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('nmsh-taxi:client:toggleMeter')
                        meterActive = false
                    end
...
-- switched boss menu from nmsh-bossmenu to taxijob
CreateThread(function()
    while true do
        local sleep = 1000
        if PlayerJob.name == "taxi" and PlayerJob.isboss and not Config.UseTarget then
            ...
            if #(pos - Config.BossMenu) < 2.0 then
                ...
                if IsControlJustReleased(0, 38) then
                   TriggerEvent('nmsh-bossmenu:client:OpenMenu')
                end
            end
        end
        Wait(sleep)
