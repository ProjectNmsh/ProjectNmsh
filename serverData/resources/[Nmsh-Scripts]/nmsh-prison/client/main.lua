...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('nmsh-clothing:client:loadOutfit', Config.Uniforms.male)
			else
				TriggerEvent('nmsh-clothing:client:loadOutfit', Config.Uniforms.female)
			end
		end)
	end
end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	...
	exports['nmsh-target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = "client",
	...
	exports['nmsh-target']:AddTargetEntity(canteen_ped, {
		options = {
			{
				type = "client",
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'nmsh-policejob' and invokingResource ~= 'nmsh-ambulancejob' and invokingResource ~= GetCurrentResourceName() then
...
RegisterNetEvent('prison:client:Leave', function()
	if jailTime > 0 then
		...
		TriggerServerEvent('nmsh-clothes:loadPlayerSkin')
