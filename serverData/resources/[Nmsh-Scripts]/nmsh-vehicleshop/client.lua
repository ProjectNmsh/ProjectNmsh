...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    TriggerServerEvent('nmsh-vehicleshop:server:addPlayer', citizenid)
    TriggerServerEvent('nmsh-vehicleshop:server:checkFinance')
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
    local citizenid = PlayerData.citizenid
    TriggerServerEvent('nmsh-vehicleshop:server:removePlayer', citizenid)
    PlayerData = {}
end)
...
local vehHeaderMenu = {
    {
        ...
        params = {
            event = 'nmsh-vehicleshop:client:showVehOptions'
        }
    }
}
...
local financeMenu = {
    {
        ...
        params = {
            event = 'nmsh-vehicleshop:client:getVehicles'
        }
    }
}
...
local returnTestDrive = {
    {
        ...
        params = {
            event = 'nmsh-vehicleshop:client:TestDriveReturn'
        }
    }
}
...
local function createTestDriveReturn()
    ...
    testDriveZone:onPlayerInOut(function(isPointInside)
        if isPointInside and IsPedInAnyVehicle(PlayerPedId()) then
            ...
            exports['nmsh-menu']:openMenu(returnTestDrive)
        else
        ...
        while inTestDrive do
            if GetGameTimer() < gameTimer + tonumber(1000 * testDriveTime) then
                ...
                if secondsLeft >= tonumber(1000 * testDriveTime) - 20 or GetPedInVehicleSeat(NetToVeh(testDriveVeh), -1) ~= PlayerPedId() then
                    TriggerServerEvent('nmsh-vehicleshop:server:deleteVehicle', testDriveVeh)
                    testDriveVeh = 0
                    inTestDrive = false
        ...
        combo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if PlayerData and PlayerData.job and (PlayerData.job.name == Config.Shops[insideShop]['Job'] or Config.Shops[insideShop]['Job'] == 'none') then
                    exports['nmsh-menu']:showHeader(vehHeaderMenu)
                end
            else
                exports['nmsh-menu']:closeMenu()
            end
                ...
                while insideShop do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'nmsh-vehicleshop:client:TestDrive',
                            }
                        },
                        {
                            ...
                            params = {
                                isServer = true,
                                event = 'nmsh-vehicleshop:server:buyShowroomVehicle',
                                args = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'nmsh-vehicleshop:client:openFinance',
                                args = {
                                    price = getVehPrice(),
                                    buyVehicle = Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle
                                }
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'nmsh-vehicleshop:client:vehCategories',
                            }
                        },
                    }
                    Wait(1000)
                ...
                while insideShop and PlayerData.job and PlayerData.job.name == Config.Shops[name]['Job'] do
                    ...
                    vehicleMenu = {
                        ...
                        },
                        {
                            ...
                            params = {
                                event = 'nmsh-vehicleshop:client:openIdMenu',
                                args = {
                        ...
                        },
                        {
                            ...
                            icon = "fa-solid fa-coins",
                            params = {
                                event = 'nmsh-vehicleshop:client:openCustomFinance',
                                args = {
                                    price = getVehPrice(),
                                    vehicle = Config.Shops[insideShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle
                                }
...
function Init()
    ...
    CreateThread(function()
        ...
        financeZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                exports['nmsh-menu']:showHeader(financeMenu)
            else
                exports['nmsh-menu']:closeMenu()
            end
        end)
...
-- Events
RegisterNetEvent('nmsh-vehicleshop:client:homeMenu', function()
    exports['nmsh-menu']:openMenu(vehicleMenu)
end)

RegisterNetEvent('nmsh-vehicleshop:client:showVehOptions', function()
    exports['nmsh-menu']:openMenu(vehicleMenu, true, true)
end)

RegisterNetEvent('nmsh-vehicleshop:client:TestDrive', function()
...
RegisterNetEvent('nmsh-vehicleshop:client:customTestDrive', function(data)
...
RegisterNetEvent('nmsh-vehicleshop:client:TestDriveReturn', function()
...
RegisterNetEvent('nmsh-vehicleshop:client:vehCategories', function()
	local catmenu = {}
    local categoryMenu = {
        {
            ...
            params = {
                event = 'nmsh-vehicleshop:client:homeMenu'
            }
        }
    }
    ...
    for k, v in pairs(catmenu) do
        categoryMenu[#categoryMenu + 1] = {
            ...
            params = {
                event = 'nmsh-vehicleshop:client:openVehCats',
                args = {
                    catName = k
                }
            }
    ...
    exports['nmsh-menu']:openMenu(categoryMenu)
end)

RegisterNetEvent('nmsh-vehicleshop:client:openVehCats', function(data)
    local vehMenu = {
        {
            header = Lang:t('menus.goback_header'),
            icon = "fa-solid fa-angle-left",
                ...
                for _, shop in pairs(QBCore.Shared.Vehicles[k]["shop"]) do
                    if shop == insideShop then
                        vehMenu[#vehMenu + 1] = {
                            ...
                            params = {
                                isServer = true,
                                event = 'nmsh-vehicleshop:server:swapVehicle',
                                args = {
                                    toVehicle = v.model,
    ...
    exports['nmsh-menu']:openMenu(vehMenu)
end)

RegisterNetEvent('nmsh-vehicleshop:client:openFinance', function(data)
    ...
    if dialog then
        ...
        TriggerServerEvent('nmsh-vehicleshop:server:financeVehicle', dialog.downPayment, dialog.paymentAmount, data.buyVehicle)
    end
end)

RegisterNetEvent('nmsh-vehicleshop:client:openCustomFinance', function(data)
    ...
    if dialog then
        ...
        TriggerServerEvent('nmsh-vehicleshop:server:sellfinanceVehicle', dialog.downPayment, dialog.paymentAmount, data.vehicle, dialog.playerid)
    end
end)

RegisterNetEvent('nmsh-vehicleshop:client:swapVehicle', function(data)
...
RegisterNetEvent('nmsh-vehicleshop:client:buyShowroomVehicle', function(vehicle, plate)
    tempShop = insideShop -- temp hacky way of setting the shop because it changes after the callback has returned since you are outside the zone
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
        local veh = NetToVeh(netId)
        exports['LegacyFuel']:SetFuel(veh, 100)
        ...
        TriggerEvent("vehiclekeys:client:SetOwner", QBCore.Functions.GetPlate(veh))
        TriggerServerEvent("nmsh-vehicletuning:server:SaveVehicleProps", QBCore.Functions.GetVehicleProperties(veh))
    end, vehicle, Config.Shops[tempShop]["VehicleSpawn"], true)
end)

RegisterNetEvent('nmsh-vehicleshop:client:getVehicles', function()
    QBCore.Functions.TriggerCallback('nmsh-vehicleshop:server:getVehicles', function(vehicles)
        local ownedVehicles = {}
        for _, v in pairs(vehicles) do
            if v.balance ~= 0 then
                ...
                ownedVehicles[#ownedVehicles + 1] = {
                    ...
                    icon = "fa-solid fa-car-side",
                    params = {
                        event = 'nmsh-vehicleshop:client:getVehicleFinance',
                        args = {
                            vehiclePlate = plate,
                            balance = v.balance,
        ...
        if #ownedVehicles > 0 then
            exports['nmsh-menu']:openMenu(ownedVehicles)
        else
...
RegisterNetEvent('nmsh-vehicleshop:client:getVehicleFinance', function(data)
    local vehFinance = {
        {
            header = Lang:t('menus.goback_header'),
            params = {
        ...
        },
        {
            ...
            icon = "fa-solid fa-hand-holding-dollar",
            params = {
                event = 'nmsh-vehicleshop:client:financePayment',
                args = {
                    vehData = data,
        ...
        },
        {
            ...
            params = {
                isServer = true,
                event = 'nmsh-vehicleshop:server:financePaymentFull',
                args = {
    ...
    exports['nmsh-menu']:openMenu(vehFinance)
end)

RegisterNetEvent('nmsh-vehicleshop:client:financePayment', function(data)
    local dialog = exports['nmsh-input']:ShowInput({
    ...
    if dialog then
        ...
        TriggerServerEvent('nmsh-vehicleshop:server:financePayment', dialog.paymentAmount, data.vehData)
    end
end)

RegisterNetEvent('nmsh-vehicleshop:client:openIdMenu', function(data)
    local dialog = exports['nmsh-input']:ShowInput({
    ...
    if dialog then
        ...
        if data.type == 'testDrive' then
            TriggerServerEvent('nmsh-vehicleshop:server:customTestDrive', data.vehicle, dialog.playerid)
        elseif data.type == 'sellVehicle' then
            TriggerServerEvent('nmsh-vehicleshop:server:sellShowroomVehicle', data.vehicle, dialog.playerid)
        end
    end
end)
