...
function resetPlateIfFake(fakePlate, veh)
    QBCore.Functions.TriggerCallback('nmsh-plateswap:server:getRealPlateFromFakePlate', function(realPlate)
        if realPlate then
...
function applyFakePlateIfExists(realPlate, veh)
    QBCore.Functions.TriggerCallback('nmsh-plateswap:server:getFakePlateFromRealPlate', function(fakePlate)
        if fakePlate then
...
local function callCops()
    ...
    TriggerServerEvent("nmsh-plateswap:server:callCops", coordinates,  streetLabel)
end
...
local function setPlate(fakePlate, vehicle)
    ...
    QBCore.Functions.TriggerCallback('nmsh-plateswap:server:setFakePlate', function(plateWasAvailable)
        if plateWasAvailable then
            SetVehicleNumberPlateText(vehicle, captializedFakePlate)
            TriggerServerEvent('nmsh-plateswap:server:setFakePlate', plate, captializedFakePlate)
            Wait(200)
            TriggerEvent('nmsh-vehiclekeys:client:AddKeys', fakePlate:upper())
        else
...
local function removeFakePlate(vehicle)
    ...
    QBCore.Functions.TriggerCallback('nmsh-plateswap:server:removeFakePlate', function(ogPlate)
...
local function takePlate(entity)
    ...
    QBCore.Functions.Progressbar("removing_plate", Lang:t('info.removing'), Config.Settings.RemoveTime, false, true, {
        ...
        QBCore.Functions.TriggerCallback('nmsh-plateswap:server:createItem', function(plateWasAvailable)
...
local function applyPlate(entity)
    ...
    QBCore.Functions.TriggerCallback('nmsh-vehiclekeys:server:GetVehicleKeys', function(keysList)
        if keysList[plate] then
            QBCore.Functions.TriggerCallback('nmsh-plateswap:server:isFakePlate', function(isFake)
                if not isFake then
                    TriggerEvent('animations:client:EmoteCommandStart', {"mechanic4"})
                    QBCore.Functions.Progressbar("applying_plate", Lang:t('info.applying'), Config.Settings.AddTime, false, true, {
                        ...
                        QBCore.Functions.TriggerCallback('nmsh-plateswap:server:getFakePlateId', function(fakePlate)
                            if fakePlate then
                                if useDebug then
...
RegisterNetEvent('nmsh-plateswap:client:setFakePlate', function(fakePlate)
    local captializedFakePlate = fakePlate:upper()
    print('fakeplate', captializedFakePlate)
...
RegisterNetEvent('nmsh-plateswap:client:removeFakePlate', function()
    local player = PlayerPedId()
...
CreateThread(function()
    ...
    exports['nmsh-target']:AddTargetBone(bones, { -- The bones can be a string or a table
      options = options,
      distance = 2.5, -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
    })
end)

RegisterNetEvent('nmsh-plateswap:client:toggleDebug', function(debug)
