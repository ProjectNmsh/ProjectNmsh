...
-- NEEDS TESTING
QBCore.Functions.CreateCallback('nmsh-plateswap:server:isFakePlate', function(source, cb, fakePlate)
...
-- NEEDS TESTING
QBCore.Functions.CreateCallback('nmsh-plateswap:server:getRealPlateFromFakePlate', function(source, cb, fakePlate)
...
QBCore.Functions.CreateCallback('nmsh-plateswap:server:getFakePlateFromRealPlate', function(source, cb, plate)
...
QBCore.Functions.CreateCallback('nmsh-plateswap:server:createItem', function(source, cb, fakePlate)
   if useDebug then
...
QBCore.Functions.CreateCallback('nmsh-plateswap:server:removeFakePlate', function(source, cb, fakePlate)
   if useDebug then
...
QBCore.Functions.CreateCallback('nmsh-plateswap:server:setFakePlate', function(source, cb, fakePlate, plate)
   if useDebug then
...
QBCore.Functions.CreateCallback('nmsh-plateswap:server:getFakePlateId', function(source, cb)
   if useDebug then
...
QBCore.Commands.Add('setfakeplate', 'Change the license plate of a car to a fake one. (Admin Only)',{ { name = 'New plate', help = 'new plate for the vehicle' } }, true, function(source, args)
   TriggerClientEvent('nmsh-plateswap:client:setFakePlate', source, args[1])
end, 'admin')

QBCore.Commands.Add('removefakeplate', 'Change the license plate of a car to a original. (Admin Only)',{}, true, function(source)
   TriggerClientEvent('nmsh-plateswap:client:removeFakePlate', source)
end, 'admin')
...
QBCore.Commands.Add('cwdebugplateswap', 'toggle debug for plateswap', {}, true, function(source, args)
    ...
    TriggerClientEvent('nmsh-plateswap:client:toggleDebug',source, useDebug)
end, 'admin')

RegisterNetEvent('nmsh-plateswap:server:callCops', function(coords, streetLabel)
   local alertData = {
