...
local function DeliverAnim()
    ...
    if Config.UseTarget and hasBag then
        ...
        exports['nmsh-target']:RemoveTargetEntity(garbageVehicle)
        if (amountOfBags - 1) <= 0 then
            ...
            exports['nmsh-target']:AddCircleZone('garbagebin', vector3(CL.coords.x, CL.coords.y, CL.coords.z), 2.0,{
...
function TakeAnim()
    ...
    QBCore.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        if Config.UseTarget and not hasBag then
            hasBag = true
            exports['nmsh-target']:RemoveZone("garbagebin")
            exports['nmsh-target']:AddTargetEntity(garbageVehicle, {
            options = {
    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("nmsh-garbagejob:client:MainMenu")
            end
            Wait(1)
        end
    end)
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['nmsh-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "nmsh-garbagejob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle', job = "garbage",}},
                distance = 2.0
            })
        else
...
RegisterNetEvent('nmsh-garbagejob:client:RequestRoute', function()
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                            ...
                            garbageVehicle = veh
                            SetVehicleNumberPlateText(veh, "nmsh-" .. tostring(math.random(1000, 9999)))
                            ...
                            TriggerServerEvent("nmsh-garbagejob:server:payDeposit")
...
RegisterNetEvent('nmsh-garbagejob:client:RequestPaycheck', function()
...
RegisterNetEvent('nmsh-garbagejob:client:MainMenu', function()
    if playerJob.name == Config.Jobname then
        local MainMenu = {}
        MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'nmsh-garbagejob:client:RequestPaycheck',}}
        if not garbageVehicle or finished then
            MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'nmsh-garbagejob:client:RequestRoute',}}
        end
        exports['nmsh-menu']:openMenu(MainMenu)
    else
