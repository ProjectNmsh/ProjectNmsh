...
-- init nmsh-target for selling spots
function spawn_seller_npc()
    ...
    for _, spot in pairs(Config.SellSpots) do
        ...
        -- init nmsh-target for sellers
        exports['nmsh-target']:AddBoxZone("sell_spot" .. _, c, 0.5, 0.5, {
            name = "sell_spot" .. _,
        ...
        },
            {
                options = {
                    {
                        event = "nmsh-hunting:client:sellREQ",
...
-- init nmsh-target for hunted animals
function putQbTargetAllOnAnimals()
    for _, v in pairs(Config.Animals) do
        exports['nmsh-target']:AddTargetModel(v.model, {
            options = { {
                icon = "fas fa-sack-dollar",
                label = "slaughter",
                ...
                action = function(entity)
                    ...
                    TriggerEvent('nmsh-hunting:client:slaughterAnimal', entity)
                    return true
                end
            } },
            distance = 1.5
