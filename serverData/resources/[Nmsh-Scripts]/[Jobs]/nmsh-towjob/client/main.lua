...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "nmsh-tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['nmsh-menu']:openMenu(towMenu)
end
...
local function CreateZone(type, number)
    ...
    elseif type == "towspots" then
        event = "nmsh-tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        exports['nmsh-target']:AddBoxZone(boxName, coords, size, size, {
            minZ = coords.z - 5.0,
            maxZ = coords.z + 5.0,
            name = boxName,
            heading = heading,
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('nmsh-tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('nmsh-tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('nmsh-tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('nmsh-tow:client:ShowMarker', true)
                else
                    TriggerEvent('nmsh-tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
RegisterNetEvent('nmsh-tow:client:SpawnVehicle', function()
...
RegisterNetEvent('nmsh-tow:client:TowVehicle', function()
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                ...
                                TriggerServerEvent('nmsh-tow:server:nano', vehNetID)
                                --remove zone
...
RegisterNetEvent('nmsh-tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('nmsh-tow:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('nmsh-tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('nmsh-tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('nmsh-tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("nmsh-tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
...
RegisterNetEvent('nmsh-tow:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('nmsh-tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)
