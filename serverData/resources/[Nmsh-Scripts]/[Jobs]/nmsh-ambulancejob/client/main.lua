...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports["nmsh-policejob"]:IsHandcuffed() then
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == "checkin" then
                   TriggerEvent('nmsh-ambulancejob:checkin')
                    listen = false
                elseif variable == "beds" then
                    TriggerEvent('nmsh-ambulancejob:beds')
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('nmsh-ambulancejob:checkin', function()
...
RegisterNetEvent('nmsh-ambulancejob:beds', function()
...
if Config.UseTarget then
    CreateThread(function()
                ...
                options = {
                    {
                        ...
                        event = "nmsh-ambulancejob:checkin",
                        label = "Check In",
                    }
                },
                distance = 1.5
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            exports['nmsh-target']:AddBoxZone("beds"..k,  v.coords, 2.5, 2.3, {
                name = "beds"..k,
                heading = -20,
                debugPoly = false,
                minZ = v.coords.z - 1,
