local QBCore = exports['nmsh-core']:GetCoreObject()
...
Citizen.CreateThread(function()
    ...
    -- Sync Image
    QBCore.Functions.TriggerCallback('nmsh-whiteboard:getCurrentImage', function(images)
    ...
    for k,v in pairs(Config.Locations) do
        ...
        exports['nmsh-target']:AddBoxZone(k..'_Target', v.Target.coords, v.Target.length, v.Target.width, {
            name = k..'_Target',
            heading = 0,
            debugPoly = false,
            minZ = v.Target.minZ,
            ...
            options = {
                {
                    type = "client",
                    event = "nmsh-whiteboard:changewhiteboardurl",
                    location = k,
                    icon = "fa fa-camera",
                    label = "Change Image",
                    job = v.job
...
AddEventHandler("nmsh-whiteboard:changewhiteboardurl", function(data)
    if data.url then
        TriggerServerEvent("nmsh-whiteboard:changewhiteboard", data.url, data.location)
    else
        ...
        if link then
            TriggerServerEvent("nmsh-whiteboard:changewhiteboard", link, data.location)
        end
    end
end)

RegisterNetEvent("nmsh-whiteboard:changewhiteboardcli", function(pUrl, pRoom)
