...
AddEventHandler('nmsh-paycheck:menu:Open_menu', function()
     QBCore.Functions.TriggerCallback('nmsh-paycheck:server:account_information', function(result)
          if result then
               Open_menu(result)
               return
          end
...
local function withdraw_all(maximum)
     ...
     QBCore.Functions.TriggerCallback('nmsh-paycheck:server:withdraw_all', function(result, reason)
          if result then
               Speach('thanks')
...
local function withdraw_amount(maximum)
     ...
     if inputData then
          ...
          QBCore.Functions.TriggerCallback('nmsh-paycheck:server:withdraw_amount', function(result, reason)
               if result then
                    TriggerEvent('animations:client:EmoteCommandStart', { "ID" })
                    QBCore.Functions.Notify(Lang:t('success.successful_withdraw') .. reason .. '$', "success")
                    Speach('thanks')
...
function menu:withdraw_menu(data)
     ...
     local req = exports['nmsh-menu']:createMenu(Menu)
     ...
     elseif req == 3 then
          ...
          QBCore.Functions.TriggerCallback('nmsh-paycheck:server:get_logs', function(result)
...
function menu:logs_menu(data)
     ...
     exports['nmsh-menu']:createMenu(Menu)
     TriggerEvent('animations:client:EmoteCommandStart', { "c" })
end
...
function QbMenu:withdraw_menu(data)
     ...
     local Menu = {
          ...
          },
          {
               ...
               icon = 'fa-solid fa-money-bill-transfer',
               params = {
                    event = 'nmsh-paycheck:client:function_caller',
          ...
          },
          {
               ...
               params = {
                    event = "nmsh-paycheck:client:close_menu",
               }
          }
     }
...
AddEventHandler('nmsh-paycheck:client:function_caller', function(data)
     TriggerEvent('animations:client:EmoteCommandStart', { "c" })
     if data.id == 1 then
          -- Withdraw All
          withdraw_all(data.money)
...
AddEventHandler('nmsh-paycheck:client:close_menu', function()
     TriggerEvent('nmsh-menu:closeMenu')
end)

AddEventHandler('nmsh-menu:closeMenu', function()
     if not GetCurrentResourceName() == 'nmsh-paycheck' then
          return
     end
     TriggerEvent('animations:client:EmoteCommandStart', { "c" })
end)
