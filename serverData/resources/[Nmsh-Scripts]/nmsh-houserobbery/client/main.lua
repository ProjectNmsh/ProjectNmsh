...
local function enterRobberyHouse(house)
    ...
    local data = exports['nmsh-interior']:CreateHouseRobbery(coords)
    ...
    if success then
        ...
        if usingAdvanced then
            if math.random(1, 100) <= Config.ChanceToBreakAdvancedLockPick then
                TriggerServerEvent("nmsh-houserobbery:server:removeAdvancedLockpick")
                TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["advancedlockpick"], "remove")
            end
        else
            if math.random(1, 100) <= Config.ChanceToBreakLockPick then
                TriggerServerEvent("nmsh-houserobbery:server:removeLockpick")
...
local function searchCabin(cabin)
    ...
    Skillbar.Start({
        ...
        if succeededAttempts + 1 >= neededAttempts then
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('nmsh-houserobbery:server:searchFurniture', cabin, currentHouse)
