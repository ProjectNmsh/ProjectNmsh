...
QBCore.Functions.CreateCallback('nmsh-vehicletuning:server:GetDrivingDistances', function(_, cb)
    cb(VehicleDrivingDistance)
end)

QBCore.Functions.CreateCallback('nmsh-vehicletuning:server:IsVehicleOwned', function(_, cb, plate)
    local retval = false
    local result = MySQL.scalar.await('SELECT 1 from player_vehicles WHERE plate = ?', {plate})
    if result then
        retval = true
...
QBCore.Functions.CreateCallback('nmsh-vehicletuning:server:GetAttachedVehicle', function(_, cb)
    cb(Config.Plates)
end)

QBCore.Functions.CreateCallback('nmsh-vehicletuning:server:IsMechanicAvailable', function(_, cb)
    local amount = 0
...
QBCore.Functions.CreateCallback('nmsh-vehicletuning:server:GetStatus', function(_, cb, plate)
...
RegisterNetEvent('nmsh-vehicletuning:server:SaveVehicleProps', function(vehicleProps)
...
RegisterNetEvent('nmsh-vehicletuning:server:UpdateDrivingDistance', function(amount, plate)
    VehicleDrivingDistance[plate] = amount
    TriggerClientEvent('nmsh-vehicletuning:client:UpdateDrivingDistance', -1, VehicleDrivingDistance[plate], plate)
...
RegisterNetEvent('nmsh-vehicletuning:server:LoadStatus', function(veh, plate)
...
RegisterNetEvent('nmsh-vehicletuning:server:SetPartLevel', function(plate, part, level)
    if VehicleStatus[plate] ~= nil then
        VehicleStatus[plate][part] = level
        TriggerClientEvent("vehiclemod:client:setVehicleStatus", -1, plate, VehicleStatus[plate])
    end
...
RegisterNetEvent('nmsh-vehicletuning:server:SetAttachedVehicle', function(veh, k)
    if veh ~= false then
        Config.Plates[k].AttachedVehicle = veh
        TriggerClientEvent('nmsh-vehicletuning:client:SetAttachedVehicle', -1, veh, k)
    else
        Config.Plates[k].AttachedVehicle = nil
        TriggerClientEvent('nmsh-vehicletuning:client:SetAttachedVehicle', -1, false, k)
    end
end)

RegisterNetEvent('nmsh-vehicletuning:server:CheckForItems', function(part)
    local src = source
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            TriggerClientEvent('nmsh-vehicletuning:client:RepaireeePart', src, part)
...
RegisterNetEvent('nmsh-mechanicjob:server:removePart', function(part, amount)
