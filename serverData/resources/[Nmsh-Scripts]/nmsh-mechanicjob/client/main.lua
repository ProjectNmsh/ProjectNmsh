...
local function DeleteTarget(id)
    if Config.UseTarget then
        exports['nmsh-target']:RemoveZone(id)
    else
...
local function RegisterDutyTarget()
    ...
    if Config.UseTarget then
        exports['nmsh-target']:AddBoxZone(dutyTargetBoxID, coords, 1.5, 1.5, {
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['nmsh-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "nmsh-mechanicjob:client:target:OpenStash",
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "nmsh-mechanicjob:client:UnattachVehicle",
            }
        }, {
            header = Lang:t('lift_menu.header_stats'),
            txt = Lang:t('lift_menu.desc_stats'),
            params = {
                event = "nmsh-mechanicjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "nmsh-mechanicjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "nmsh-mechanicjob:client:target:CloseMenu",
            }
        },
    }
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "nmsh-mechanicjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    txt = Lang:t('parts_menu.status') .. percentage .. ".0% / 100.0%",
                    params = {
                        event = "nmsh-mechanicjob:client:NoDamage",
                    }
                }
            end
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        },
        {
            ...
            params = {
                event = "nmsh-mechanicjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
    ...
    exports['nmsh-menu']:openMenu(TestMenu1)
end
...
local function NoDamage()
    ...
    exports['nmsh-menu']:openMenu(noDamage)
end
...
local function UnattachVehicle()
    ...
    TriggerServerEvent('nmsh-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
...
local function VehicleList()
    ...
    for k,v in pairs(Config.Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "Vehicle: "..v.."",
            params = {
                event = "nmsh-mechanicjob:client:SpawnListVehicle",
                args = {
                    headername = v,
                    spawnName = k
                }
...
local function RepairPart(part)
    ...
    local indx = 0
    local countitem = 0
    QBCore.Functions.TriggerCallback('nmsh-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", Lang:t('labels.progress_bar') ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerEvent('nmsh-vehicletuning:client:RepaireeePart', part)
                TriggerServerEvent('nmsh-inventory:server:SaveStashItems', "mechanicstash", StashItems)
...
RegisterNetEvent("nmsh-mechanicjob:client:UnattachVehicle",function()
    UnattachVehicle()
end)

RegisterNetEvent("nmsh-mechanicjob:client:PartsMenu",function()
    PartsMenu()
end)

RegisterNetEvent("nmsh-mechanicjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("nmsh-mechanicjob:client:NoDamage",function()
    NoDamage()
end)

RegisterNetEvent("nmsh-mechanicjob:client:CheckStatus",function()
    CheckStatus()
end)

RegisterNetEvent("nmsh-mechanicjob:client:SpawnListVehicle",function(data)
    local vehicleSpawnName = data.spawnName
    SpawnListVehicle(vehicleSpawnName)
end)

RegisterNetEvent("nmsh-mechanicjob:client:RepairPart",function(data)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    QBCore.Functions.TriggerCallback('nmsh-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    QBCore.Functions.TriggerCallback('nmsh-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('nmsh-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
...
RegisterNetEvent('nmsh-vehicletuning:client:RepaireeePart', function(part)
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        QBCore.Functions.Progressbar("repair_advanced", Lang:t('notifications.progress_bar'), lockpickTime, false, true, {
                            ...
                            if part == "body" then
                                ...
                                TriggerServerEvent("nmsh-mechanicjob:server:removePart", part, needAmount)
                            ...
                            elseif part ~= "engine" then
                                ...
                                TriggerServerEvent("nmsh-mechanicjob:server:removePart", part, level)
                                TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[Config.RepairCost[part]], "remove")
                            end
                        end, function() -- Cancel
                            openingDoor = false
...
RegisterNetEvent('nmsh-mechanicjob:client:target:OpenStash', function ()
...
RegisterNetEvent('nmsh-mechanicjob:client:target:CloseMenu', function()
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])

    TriggerEvent('nmsh-menu:client:closeMenu')
end)
        ...
        if PlayerJob.type == 'mechanic' then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 0
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("nmsh-mechanicjob:client:target:OpenStash")
                    end
                end
                ...
                if isInsideVehiclePlateZone then
                    ...
                    if attachedVehicle then
                        ...
                        if IsControlJustPressed(0, 38) and IsPedInAnyVehicle(PlayerPedId()) then
                            ...
                            Wait(500)
                            DoScreenFadeIn(150)
                            TriggerServerEvent('nmsh-vehicletuning:server:SetAttachedVehicle', veh, ClosestPlate)
