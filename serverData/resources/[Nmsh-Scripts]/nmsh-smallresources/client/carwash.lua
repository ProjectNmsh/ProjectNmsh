            ...
            if Driver == PlayerPed and not washingVehicle then
                if IsControlPressed(0, 38) then
                    if dirtLevel > Config.DirtLevel then
                        TriggerServerEvent('nmsh-carwash:server:washCar')
                    else
...
RegisterNetEvent('nmsh-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed, false)
    washingVehicle = true
...
CreateThread(function()
    for k,v in pairs(Config.CarWash) do
        if Config.UseTarget then
            exports["nmsh-target"]:AddBoxZone('carwash_'..k, v['poly'].coords, v['poly'].length, v['poly'].width, {
                name = 'carwash_'..k,
                debugPoly = false,
