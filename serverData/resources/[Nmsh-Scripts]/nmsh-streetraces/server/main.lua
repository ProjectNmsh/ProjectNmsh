...
RegisterNetEvent('nmsh-streetraces:NewRace', function(RaceTable)
    local src = source
    ...
    if xPlayer.Functions.RemoveMoney('cash', RaceTable.amount, "streetrace-created") then
        ...
        TriggerClientEvent('nmsh-streetraces:SetRace', -1, Races)
        TriggerClientEvent('nmsh-streetraces:SetRaceId', src, RaceId)
        TriggerClientEvent('QBCore:Notify', src, "You joind the race for €"..Races[RaceId].amount..",-", 'success')
    end
end)

RegisterNetEvent('nmsh-streetraces:RaceWon', function(RaceId)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('nmsh-streetraces:SetRace', -1, Races)
    TriggerClientEvent('nmsh-streetraces:RaceDone', -1, RaceId, GetPlayerName(src))
end)

RegisterNetEvent('nmsh-streetraces:JoinRace', function(RaceId)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add("createrace", "Start A Street Race", {{name="amount", help="The Stake Amount For The Race."}}, false, function(source, args)
    ...
    if GetJoinedRace(QBCore.Functions.GetIdentifier(src, 'license')) == 0 then
        TriggerClientEvent('nmsh-streetraces:CreateRace', src, amount)
    else
...
QBCore.Commands.Add("startrace", "Start The Race", {}, false, function(source)
    ...
    if RaceId ~= 0 then
        ...
        TriggerClientEvent('nmsh-streetraces:SetRace', -1, Races)
        TriggerClientEvent("nmsh-streetraces:StartRace", -1, RaceId)
    else
                ...
                if not Races[key].started then
                    for _, iden in pairs(Races[key].joined) do
                        local xdPlayer = QBCore.Functions.GetPlayer(iden)
                            ...
                            TriggerClientEvent('nmsh-streetraces:StopRace', xdPlayer.PlayerData.source)
                            RemoveFromRace(iden)
                    end
                else
