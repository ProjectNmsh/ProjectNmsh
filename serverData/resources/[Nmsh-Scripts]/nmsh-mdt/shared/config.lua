...
-- However, if you're using a different inventory system, please refer to the "Inventory Edit | Automatic Add Weapons with images" section in nmsh-mdt's README.
Config.InventoryForWeaponsImages = "nmsh-inventory"

-- "LegacyFuel", "lj-fuel", "nmsh-fuel"
...
-- This is a workaround solution because the nmsh-menu present in nmsh-policejob fills in an impound location and sends it to the event. 
-- If the impound locations are modified in nmsh-policejob, the changes must also be implemented here to ensure consistency.
