...
RegisterNetEvent('nmsh-admin:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('nmsh-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('nmsh-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('nmsh-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('nmsh-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('nmsh-admin:server:SaveCar', props, QBCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
...
RegisterNetEvent('nmsh-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('nmsh-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('nmsh-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = PlayerPedId()
...
RegisterNetEvent('nmsh-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('nmsh-admin:client:maxmodVehicle', function()
