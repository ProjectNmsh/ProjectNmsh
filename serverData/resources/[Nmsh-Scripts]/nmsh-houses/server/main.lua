...
CreateThread(function()
    ...
    TriggerClientEvent("nmsh-garages:client:houseGarageConfig", -1, HouseGarages)
    TriggerClientEvent("nmsh-houses:client:setHouseConfig", -1, Config.Houses)
end)
...
QBCore.Commands.Add("decorate", Lang:t("info.decorate_interior"), {}, false, function(source)
    local src = source
    TriggerClientEvent("nmsh-houses:client:decorate", src)
end)
...
QBCore.Commands.Add("createhouse", Lang:t("info.create_house"), {{name = "price", help = Lang:t("info.price_of_house")}, {name = "tier", help = Lang:t("info.tier_number")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("nmsh-houses:client:createHouses", src, price, tier)
    else
...
QBCore.Commands.Add("addgarage", Lang:t('info.add_garage'), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("nmsh-houses:client:addGarage", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.realestate_only"), "error")
    end
end)
...
QBCore.Commands.Add("ring", Lang:t("info.ring_doorbell"), {}, false, function(source)
    local src = source
    TriggerClientEvent('nmsh-houses:client:RequestRing', src)
end)
...
QBCore.Functions.CreateUseableItem("police_stormram", function(source, _)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        TriggerClientEvent("nmsh-houses:client:HomeInvasion", source)
    else
...
RegisterNetEvent('nmsh-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent("nmsh-houses:client:setHouseConfig", src, Config.Houses)
end)

RegisterNetEvent('nmsh-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent("nmsh-houses:client:createBlip", -1, coords)
end)

RegisterNetEvent('nmsh-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", "")
    ...
    TriggerEvent('nmsh-log:server:CreateLog', 'house', Lang:t("log.house_created"), 'green', Lang:t("log.house_address", {label = label, price = price, tier = tier, agent = GetPlayerName(src)}))
end)

RegisterNetEvent('nmsh-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent("nmsh-garages:client:addHouseGarage", -1, house, garageInfo)
    TriggerClientEvent('QBCore:Notify', src, Lang:t("info.added_garage", {value = garageInfo.label}))
end)

RegisterNetEvent('nmsh-houses:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('nmsh-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes,
        pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('nmsh-houses:server:buyHouse', function(house)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('nmsh-houses:client:SetClosestHouse', src)
        TriggerClientEvent('nmsh-house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, "bought-house") -- 21% Extra house costs
        exports['nmsh-management']:AddMoney("realestate", (HousePrice / 100) * math.random(18, 25))
        TriggerEvent('nmsh-log:server:CreateLog', 'house', Lang:t("log.house_purchased"), 'green', Lang:t("log.house_purchased_by", {house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname}))
...
RegisterNetEvent('nmsh-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('nmsh-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('nmsh-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('nmsh-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('nmsh-houses:server:giveKey', function(house, target)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('nmsh-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = QBCore.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('nmsh-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('nmsh-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('nmsh-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('nmsh-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', {json.encode(decorations), house})
    TriggerClientEvent("nmsh-houses:server:sethousedecorations", -1, house, decorations)
end)

RegisterNetEvent('nmsh-houses:server:LogoutLocation', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('nmsh-multicharacter:client:chooseChar', src)
end)

RegisterNetEvent('nmsh-houses:server:giveHouseKey', function(target, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if housekeyholders[house] then
        ...
        MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
        TriggerClientEvent('nmsh-houses:client:refreshHouse', tPlayer.PlayerData.source)
...
RegisterNetEvent('nmsh-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('nmsh-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('nmsh-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('nmsh-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('nmsh-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('nmsh-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money["bank"]
...
QBCore.Functions.CreateCallback('nmsh-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
QBCore.Functions.CreateCallback('nmsh-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
QBCore.Functions.CreateCallback('nmsh-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('nmsh-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

QBCore.Functions.CreateCallback('nmsh-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
QBCore.Functions.CreateCallback('nmsh-phone:server:TransferCid', function(_, cb, NewCid, house)
...
QBCore.Functions.CreateCallback('nmsh-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
QBCore.Functions.CreateCallback('nmsh-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', {house})
    if result[1] then
        retval = result[1]
...
QBCore.Functions.CreateCallback('nmsh-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    if pData then
...
QBCore.Functions.CreateCallback('nmsh-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('nmsh-phone:server:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local MyHouses = {}
...
QBCore.Functions.CreateCallback('nmsh-phone:server:GetHouseKeys', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local MyKeys = {}
...
QBCore.Functions.CreateCallback('nmsh-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then
