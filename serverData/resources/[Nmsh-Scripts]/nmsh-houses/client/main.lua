...
local function showEntranceHeaderMenu()
    ...
    if not isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "nmsh-houses:client:ViewHouse",
                args = {}
            }
        }
    else
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "nmsh-houses:client:EnterHouse",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "nmsh-houses:client:giveHouseKey",
                    args = {}
                }
            }
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "nmsh-houses:client:RequestRing",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
            ...
            if QBCore.Functions.GetPlayerData().job and QBCore.Functions.GetPlayerData().job.name == 'police' then
                headerMenu[#headerMenu+1] = {
                    ...
                    params = {
                        event = "nmsh-houses:client:ResetHouse",
                        args = {}
                    }
                }
            end
    ...
    if headerMenu and next(headerMenu) then
        exports['nmsh-menu']:openMenu(headerMenu)
    end
end
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "nmsh-houses:client:ExitOwnedHouse",
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "nmsh-houses:client:FrontDoorCam",
                args = {}
            }
        }
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "nmsh-houses:client:AnswerDoorbell",
                args = {}
            }
        }
    end
...
RegisterNetEvent('nmsh-houses:client:setHouseConfig', function(houseConfig)
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            QBCore.Functions.TriggerCallback('nmsh-houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    end
    TriggerEvent('nmsh-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        QBCore.Functions.TriggerCallback('nmsh-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		QBCore.Functions.TriggerCallback('nmsh-houses:server:getHouseDecorations', function(result)
...
local function RemoveHouseKey(citizenData)
    TriggerServerEvent('nmsh-houses:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    QBCore.Functions.TriggerCallback('nmsh-houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end,ClosestHouse)
    ...
    if holders == nil or next(holders) == nil then
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu+1] = {
                ...
                params = {
                    event = "nmsh-houses:client:OpenClientOptions",
                    args = {
        ...
        exports['nmsh-menu']:openMenu(keyholderMenu)
    end
    ...
    keyholderOptions = {
        {
            ...
            params = {
                event = "nmsh-houses:client:RevokeKey",
                args = {
        ...
        },
        {
            ...
            params = {
                event = "nmsh-houses:client:removeHouseKey",
                args = {}
            }
        },
    }

    exports['nmsh-menu']:openMenu(keyholderOptions)
end
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['nmsh-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['nmsh-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['nmsh-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['nmsh-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['nmsh-interior']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['nmsh-interior']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['nmsh-interior']:CreateContainer(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['nmsh-interior']:CreateFurniMid(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['nmsh-interior']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['nmsh-interior']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['nmsh-interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['nmsh-interior']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['nmsh-interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['nmsh-interior']:CreateStore1(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['nmsh-interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['nmsh-interior']:CreateFurniMotelStandard(coords) -- End of free shells
    elseif Config.Houses[house].tier == 17 then
        return exports['nmsh-interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['nmsh-interior']:CreateMedium3(coords)
    elseif Config.Houses[house].tier == 19 then
        return exports['nmsh-interior']:CreateBanham(coords)
    elseif Config.Houses[house].tier == 20 then
        return exports['nmsh-interior']:CreateWestons(coords)
    elseif Config.Houses[house].tier == 21 then
        return exports['nmsh-interior']:CreateWestons2(coords)
    elseif Config.Houses[house].tier == 22 then
        return exports['nmsh-interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 23 then
        return exports['nmsh-interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 24 then
        return exports['nmsh-interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 25 then
        return exports['nmsh-interior']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 26 then
        return exports['nmsh-interior']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 27 then
        return exports['nmsh-interior']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 28 then
        return exports['nmsh-interior']:CreateHighend(coords)
    elseif Config.Houses[house].tier == 29 then
        return exports['nmsh-interior']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 30 then
        return exports['nmsh-interior']:CreateStashHouse(coords)
    elseif Config.Houses[house].tier == 31 then
        return exports['nmsh-interior']:CreateStashHouse2(coords)
    elseif Config.Houses[house].tier == 32 then
        return exports['nmsh-interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 33 then
        return exports['nmsh-interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 34 then
        return exports['nmsh-interior']:CreateOffice2(coords)
    elseif Config.Houses[house].tier == 35 then
        return exports['nmsh-interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 36 then
        return exports['nmsh-interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 37 then
        return exports['nmsh-interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 38 then
        return exports['nmsh-interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 39 then
        return exports['nmsh-interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 40 then
        return exports['nmsh-interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 41 then
        return exports['nmsh-interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 42 then
        return exports['nmsh-interior']:CreateK4Coke(coords)
    elseif Config.Houses[house].tier == 43 then
        return exports['nmsh-interior']:CreateK4Meth(coords)
    elseif Config.Houses[house].tier == 44 then
        return exports['nmsh-interior']:CreateK4Weed(coords)
    elseif Config.Houses[house].tier == 45 then
        return exports['nmsh-interior']:CreateContainer2(coords)
    elseif Config.Houses[house].tier == 46 then
        return exports['nmsh-interior']:CreateFurniStash1(coords)
    elseif Config.Houses[house].tier == 47 then
        return exports['nmsh-interior']:CreateFurniStash3(coords)
    elseif Config.Houses[house].tier == 48 then
        return exports['nmsh-interior']:CreateFurniLow(coords)
    elseif Config.Houses[house].tier == 49 then
        return exports['nmsh-interior']:CreateFurniMotel(coords)
    elseif Config.Houses[house].tier == 50 then
        return exports['nmsh-interior']:CreateFurniMotelClassic(coords)
    elseif Config.Houses[house].tier == 51 then
        return exports['nmsh-interior']:CreateFurniMotelHigh(coords)
    elseif Config.Houses[house].tier == 52 then
        return exports['nmsh-interior']:CreateFurniMotelModern2(coords)
    elseif Config.Houses[house].tier == 53 then
        return exports['nmsh-interior']:CreateFurniMotelModern3(coords)
    elseif Config.Houses[house].tier == 54 then
        return exports['nmsh-interior']:CreateCoke(coords)
    elseif Config.Houses[house].tier == 55 then
        return exports['nmsh-interior']:CreateCoke2(coords)
    elseif Config.Houses[house].tier == 56 then
        return exports['nmsh-interior']:CreateMeth(coords)
    elseif Config.Houses[house].tier == 57 then
        return exports['nmsh-interior']:CreateWeed(coords)
    elseif Config.Houses[house].tier == 58 then
        return exports['nmsh-interior']:CreateWeed2(coords)
    elseif Config.Houses[house].tier == 59 then
        return exports['nmsh-interior']:CreateMansion(coords)
    elseif Config.Houses[house].tier == 60 then
        return exports['nmsh-interior']:CreateMansion2(coords)
    elseif Config.Houses[house].tier == 61 then
        return exports['nmsh-interior']:CreateMansion3(coords)
    elseif Config.Houses[house].tier == 62 then
        return exports['nmsh-interior']:CreateHotel1(coords)
    elseif Config.Houses[house].tier == 63 then
        return exports['nmsh-interior']:CreateHotel2(coords)
    elseif Config.Houses[house].tier == 64 then
        return exports['nmsh-interior']:CreateHotel3(coords)
    elseif Config.Houses[house].tier == 65 then
        return exports['nmsh-interior']:CreateMotel1(coords)
    elseif Config.Houses[house].tier == 66 then
        return exports['nmsh-interior']:CreateMotel2(coords)
    elseif Config.Houses[house].tier == 67 then
        return exports['nmsh-interior']:CreateMotel3(coords)
    elseif Config.Houses[house].tier == 68 then
        return exports['nmsh-interior']:CreateV2Default1(coords)
    elseif Config.Houses[house].tier == 69 then
        return exports['nmsh-interior']:CreateV2Default2(coords)
    elseif Config.Houses[house].tier == 70 then
        return exports['nmsh-interior']:CreateV2Default3(coords)
    elseif Config.Houses[house].tier == 71 then
        return exports['nmsh-interior']:CreateV2Default4(coords)
    elseif Config.Houses[house].tier == 72 then
        return exports['nmsh-interior']:CreateV2Default5(coords)
    elseif Config.Houses[house].tier == 73 then
        return exports['nmsh-interior']:CreateV2Default6(coords)
    elseif Config.Houses[house].tier == 74 then
        return exports['nmsh-interior']:CreateV2Deluxe1(coords)
    elseif Config.Houses[house].tier == 75 then
        return exports['nmsh-interior']:CreateV2Deluxe2(coords)
    elseif Config.Houses[house].tier == 76 then
        return exports['nmsh-interior']:CreateV2Deluxe3(coords)
    elseif Config.Houses[house].tier == 77 then
        return exports['nmsh-interior']:CreateV2HighEnd1(coords)
    elseif Config.Houses[house].tier == 78 then
        return exports['nmsh-interior']:CreateV2HighEnd2(coords)
    elseif Config.Houses[house].tier == 79 then
        return exports['nmsh-interior']:CreateV2HighEnd3(coords)
    elseif Config.Houses[house].tier == 80 then
        return exports['nmsh-interior']:CreateV2Medium1(coords)
    elseif Config.Houses[house].tier == 81 then
        return exports['nmsh-interior']:CreateV2Medium2(coords)
    elseif Config.Houses[house].tier == 82 then
        return exports['nmsh-interior']:CreateV2Medium3(coords)
    elseif Config.Houses[house].tier == 83 then
        return exports['nmsh-interior']:CreateV2Modern1(coords)
    elseif Config.Houses[house].tier == 84 then
        return exports['nmsh-interior']:CreateV2Modern2(coords)
    elseif Config.Houses[house].tier == 85 then
        return exports['nmsh-interior']:CreateV2Modern3(coords)
    else
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Wait(500)
    TriggerServerEvent('nmsh-houses:server:SetInsideMeta', house, true)
    --TriggerEvent('nmsh-weathersync:client:DisableSync')
    TriggerEvent('nmsh-weathersync:client:EnableSync')
    TriggerEvent('nmsh-weed:client:getHousePlants', house)
    entering = false
    setHouseLocations()
    CloseMenuFull()
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        exports['nmsh-interior']:DespawnInterior(houseObj, function()
            ...
            TriggerEvent('nmsh-weed:client:leaveHouse')
            TriggerServerEvent('nmsh-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
local function changeCharacter()
    ...
    if CheckDistance(logoutLoc, 1.5) then
        ...
        exports['nmsh-interior']:DespawnInterior(houseObj, function()
            ...
            InOwnedHouse = false
            IsInside = false
            TriggerServerEvent('nmsh-houses:server:LogoutLocation')
        end)
    end
end
-- Events

RegisterNetEvent('nmsh-houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('nmsh-houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('nmsh-houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('nmsh-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('nmsh-houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('nmsh-houses:server:RingDoor', ClosestHouse)
    end
end)

AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent('nmsh-houses:server:setHouses')
    SetClosestHouse()
    TriggerEvent('nmsh-houses:client:setupHouseBlips')
    if Config.UnownedBlips then TriggerEvent('nmsh-houses:client:setupHouseBlips2') end
    Wait(100)
    TriggerEvent('nmsh-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
    TriggerServerEvent("nmsh-houses:server:setHouses")
end)
...
RegisterNetEvent('nmsh-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('nmsh-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('nmsh-houses:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('nmsh-houses:server:createBlip') end
end)

RegisterNetEvent('nmsh-houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('nmsh-houses:server:addGarage', ClosestHouse, coords)
    else
        QBCore.Functions.Notify(Lang:t("error.no_house"), "error")
    end
end)

RegisterNetEvent('nmsh-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('nmsh-houses:server:lockHouse', false, ClosestHouse)
                QBCore.Functions.Notify(Lang:t("success.unlocked"), "success", 2500)
            else
                TriggerServerEvent('nmsh-houses:server:lockHouse', true, ClosestHouse)
...
RegisterNetEvent('nmsh-houses:client:RingDoor', function(player, house)
...
RegisterNetEvent('nmsh-houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('nmsh-houses:server:giveHouseKey', playerId, ClosestHouse)
        else
...
RegisterNetEvent('nmsh-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            QBCore.Functions.TriggerCallback('nmsh-houses:server:getHouseOwner', function(result)
...
RegisterNetEvent('nmsh-houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('nmsh-houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('nmsh-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('nmsh-houses:client:enterOwnedHouse', function(house)
...
RegisterNetEvent('nmsh-houses:client:LastLocationHouse', function(houseId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
RegisterNetEvent('nmsh-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            QBCore.Functions.TriggerCallback('nmsh-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('nmsh-houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('nmsh-houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('nmsh-houses:client:refreshBlips', function() -- Refresh unowned on buy
    for _, v in pairs(UnownedHouseBlips) do RemoveBlip(v) end
    Wait(250)
    TriggerEvent('nmsh-houses:client:setupHouseBlips2')
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('nmsh-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('nmsh-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('nmsh-houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if cData.id == 'setstash' then
                TriggerServerEvent('nmsh-houses:server:setLocation', coords, ClosestHouse, 1)
            elseif cData.id == 'setoutift' then
                TriggerServerEvent('nmsh-houses:server:setLocation', coords, ClosestHouse, 2)
            elseif cData.id == 'setlogout' then
                TriggerServerEvent('nmsh-houses:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            QBCore.Functions.Notify(Lang:t("error.not_owner"), "error")
        end
...
RegisterNetEvent('nmsh-houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('nmsh-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
                        ...
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    ...
                                    TriggerServerEvent('nmsh-houses:server:SetHouseRammed', true, ClosestHouse)
                                    TriggerServerEvent('nmsh-houses:server:SetRamState', false, ClosestHouse)
                            ...
                            TriggerServerEvent('nmsh-houses:server:SetRamState', true, ClosestHouse)
                        else
...
RegisterNetEvent('nmsh-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('nmsh-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('nmsh-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('nmsh-houses:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('nmsh-houses:server:SetRamState', false, ClosestHouse)
        end
...
RegisterNetEvent('nmsh-houses:client:ExitOwnedHouse', function()
...
RegisterNetEvent('nmsh-houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterNetEvent('nmsh-houses:client:AnswerDoorbell', function()
    ...
    if CheckDistance(door, 1.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent("nmsh-houses:server:OpenDoor", CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('nmsh-houses:client:ViewHouse', function()
...
RegisterNetEvent('nmsh-houses:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterNetEvent('nmsh-house:client:RefreshHouseTargets', function ()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    QBCore.Functions.TriggerCallback('nmsh-houses:server:HasEnoughMoney', function(_)
        cb('ok')
...
RegisterNUICallback('buy', function(_, cb)
    ...
    if Config.UnownedBlips then TriggerEvent('nmsh-houses:client:refreshBlips') end
    TriggerServerEvent('nmsh-houses:server:buyHouse', ClosestHouse)
    cb("ok")
end)
