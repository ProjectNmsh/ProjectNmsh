...
local function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('nmsh-anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("nmsh-houses:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('nmsh-anticheat:client:ToggleDecorate', false)
end
...
RegisterNetEvent('nmsh-houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    QBCore.Functions.TriggerCallback('nmsh-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()
