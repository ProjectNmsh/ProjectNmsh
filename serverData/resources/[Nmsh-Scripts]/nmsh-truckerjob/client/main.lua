...
--    if PlayerData.metadata.jobrep.trucker >= v.jobrep then
        ...
        for k,v in pairs(Config.TruckerJobVehicles) do
            truckMenu[#truckMenu+1] = {
             header = v.label,
             params = {
                 event = "nmsh-trucker:client:TakeOutVehicle",
                 args = {
                     vehicle = k
                   }
                }
        ...
        exports['nmsh-menu']:openMenu(truckMenu)
--    end
end
...
local function CreateZone(type, number)
    ...
    if type == "main" then
        event = "nmsh-truckerjob:client:PaySlip"
        label = "Payslip"
    ...
    elseif type == "vehicle" then
        event = "nmsh-truckerjob:client:Vehicle"
        label = "Vehicle"
        coords = vector3(Config.TruckerJobLocations[type].coords.x, Config.TruckerJobLocations[type].coords.y, Config.TruckerJobLocations[type].coords.z)
        heading = Config.TruckerJobLocations[type].coords.h
        boxName = Config.TruckerJobLocations[type].label
    ...
    elseif type == "stores" then
        event = "nmsh-truckerjob:client:Store"
        label = "Store"
    ...
    elseif type == "line-haul" then
        event = "nmsh-truckerjob:client:Line-Haul"
        label = "Line-Haul"
        coords = vector3(Config.TruckerJobLocations[type][number].coords.x, Config.TruckerJobLocations[type][number].coords.y, Config.TruckerJobLocations[type][number].coords.z)
        heading = Config.TruckerJobLocations[type][number].coords.h
        boxName = Config.TruckerJobLocations[type][number].name
    ...
    if Config.UseTarget and type == "main" then
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('nmsh-truckerjob:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('nmsh-truckerjob:client:Vehicle')
                elseif type == "stores" then
...
local function Deliver()
    ...
    QBCore.Functions.Progressbar("work_dropbox", Lang:t("mission.deliver_box"), 2000, false, true, {
        ...
        if currentCount == CurrentLocation.dropcount then
            ...
            TriggerServerEvent("nmsh-shops:server:RestockShopItems", CurrentLocation.store)
            exports['qb-core']:HideText()
            Delivering = false
            showMarker = false
            TriggerServerEvent('nmsh-trucker:server:nano')
            if CurrentBlip ~= nil then
                RemoveBlip(CurrentBlip)
                ClearAllBlipRoutes()
                CurrentBlip = nil
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob.name ~= "trucker" then return end
    CreateElements()
    TriggerServerEvent('nmsh-shops:server:SetShopList')
end)
...
RegisterNetEvent('nmsh-trucker:client:SpawnVehicle', function()
...
RegisterNetEvent('nmsh-trucker:client:TakeOutVehicle', function(data)
    local vehicleInfo = data.vehicle
    TriggerServerEvent('nmsh-trucker:server:DoBail', true, vehicleInfo)
    selectedVeh = vehicleInfo
end)

RegisterNetEvent('nmsh-truckerjob:client:Vehicle', function()
    if IsPedInAnyVehicle(PlayerPedId()) and isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
        if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
            if isTruckerVehicle(GetVehiclePedIsIn(PlayerPedId(), false)) then
                ...
                TriggerServerEvent('nmsh-trucker:server:DoBail', false)
                if CurrentBlip ~= nil then
                    RemoveBlip(CurrentBlip)
                    ClearAllBlipRoutes()
                    CurrentBlip = nil
...
RegisterNetEvent('nmsh-truckerjob:client:PaySlip', function()
    if JobsDone > 0 then
        TriggerServerEvent("nmsh-trucker:server:01101110", JobsDone)
        JobsDone = 0
...
RegisterNetEvent('nmsh-truckerjob:client:SetShopList', function(shoplist)
