...
Close = function() {
    ...
    $.post('https://nmsh-cityhall/close');
...
$('.cityhall-option-block').click(function(e){
    ...
    if (blockPage == "identity") {
        ...
        $.post('https://nmsh-cityhall/requestLicenses', JSON.stringify({}), function(licenses){
...
$(".request-identity-button").click(function(e){
    e.preventDefault();
    $.post('https://nmsh-cityhall/requestId', JSON.stringify({
...
$(document).on('click', '.apply-job-button', function(e){
    e.preventDefault();
    $.post('https://nmsh-cityhall/applyJob', JSON.stringify(selectedJobId))
    ResetPages();
});
