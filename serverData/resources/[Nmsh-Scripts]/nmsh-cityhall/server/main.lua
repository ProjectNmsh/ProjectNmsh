...
QBCore.Functions.CreateCallback('nmsh-cityhall:server:receiveJobs', function(_, cb)
    cb(availableJobs)
end)

-- Events

RegisterNetEvent('nmsh-cityhall:server:requestId', function(item, hall)
    local src = source
...
RegisterNetEvent('nmsh-cityhall:server:sendDriverTest', function(instructors)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    for i = 1, #instructors do
        ...
        if SchoolPlayer then
            TriggerClientEvent("nmsh-cityhall:client:sendDriverEmail", SchoolPlayer.PlayerData.source, Player.PlayerData.charinfo)
        else
            local mailData = {
                sender = "Township",
            ...
            exports["nmsh-phone"]:sendNewMailToOffline(citizenid, mailData)
        end
    end
...
RegisterNetEvent('nmsh-cityhall:server:ApplyJob', function(job, cityhallCoords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if QBCore.Shared.QBJobsStatus then
        exports["nmsh-jobs"]:submitApplication(data, "Jobs")
    else
...
RegisterNetEvent('nmsh-cityhall:server:getIDs', giveStarterItems)
