...
local function getJobs()
    QBCore.Functions.TriggerCallback('nmsh-cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('nmsh-cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
...
RegisterNetEvent('nmsh-cityhall:Client:AddCityJob', function()
    getJobs()
end)

RegisterNetEvent('nmsh-cityhall:client:getIds', function()
    TriggerServerEvent('nmsh-cityhall:server:getIDs')
end)

RegisterNetEvent('nmsh-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('nmsh-cityhall:server:requestId', id.type, closestCityhall)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('nmsh-cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        QBCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
