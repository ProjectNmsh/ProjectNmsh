local repairPoints = {
    vector3(-106.2314, -2512.425, 4.8460683),
}

local testDrivePoints = {
    vector4(-126.909, -2536.551, 6.0000014, 146.41722)
}
...
/*Citizen.CreateThread(function()
    ...
    while true do
        ...
        local dist = #(GetEntityCoords(PlayerPedId()) - vector3(-109.4501, -2519.125, 5.9999947))
        local veh = GetVehiclePedIsIn(player, false)
        local health = GetVehicleBodyHealth(veh)
        if dist < 1.5 then
  
            DrawText3D(-109.4501, -2519.125, 5.9999947, "[E] REPAIR")
            if IsControlJustPressed(1, 38) then
                ...
                if not isPurchaseSuccessful then
                    ...
                    local finished = exports["ks-taskbar"]:taskBar(15000,"Fixing Vehicle",true)
...
Citizen.CreateThread(function()
    while true do
        ...
        if insideDriftSchool then
            ...
            local veh = GetVehiclePedIsIn(player, false)
            local health = GetVehicleBodyHealth(veh)
            if repairDist[1] < 5 and health < 1000.0 and veh ~= 0 then
                QBCore.Functions.DrawText3D(repairDist[2].x, repairDist[2].y, repairDist[2].z, "[E] Repair Vehicle $"..repairCost(veh, health))
            ...
            elseif testDriveDist[1] < 5 then
                -- testdrive menu
                local point = testDriveDist[2]
                if veh == 0 then
                    QBCore.Functions.DrawText3D(point.x, point.y, point.z, "[E] Open Test Drives")
                else
                    QBCore.Functions.DrawText3D(point.x, point.y, point.z, "[E] Put away Test Drive")
                end
...
RegisterNetEvent('open:drift',function()
    openGui(true)
end)

RegisterNetEvent('open:repair',function()
    openGui(true)
end)
    ...
    Citizen.CreateThread(function()
        ...
        if not DoesEntityExist(veh) then
            if IsModelInCdimage(model) and IsModelValid(model) then
                ...
                QBCore.Functions.Notify('Error spawning car.')
                return
            end

            veh = CreateVehicle(model, -121.7162, -2533.965, 5.9999985, 236.69937, true, false)
            ...
            exports['ks-fuel']:SetFuel(veh, 100.0)
	        local plate = GetVehicleNumberPlateText(veh)	
	        --TriggerEvent("vehiclekeys:client:SetOwner", plate)  
        else
            --TriggerEvent("DoLongHudText","A car is on the spawn point.",2)
            QBCore.Functions.Notify('A car is on the spawn point..')
        end
    end)
    cb('ok')
end)
